...
const noLayers=calculate({...base,max_total_height_mm:300});
assert.equal(noLayers.layers,0);
assert.equal(noLayers.boxes_per_pallet,0);
assert.equal(noLayers.load_height_mm,144);

const oversized=calculate({...base,box_length_mm:1300,box_width_mm:900});
assert.equal(oversized.boxes_per_layer,0);
assert.equal(oversized.boxes_per_pallet,0);

assert.throws(()=>calculate({...base,max_total_height_mm:144}),/Maximum height must exceed pallet height/);
